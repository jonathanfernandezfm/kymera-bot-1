...
module.exports = {
	...
	execute: async (interaction) => {
		...
		const row = new ActionRowBuilder().addComponents(
			new ButtonBuilder().setCustomId('aceptar_formulario_ali').setLabel('Aceptar alianza').setStyle(ButtonStyle.Success),
			new ButtonBuilder().setCustomId('denegar_formulario_ali').setLabel('Denegar alianza').setStyle(ButtonStyle.Danger)
		);
		...
		const embed = new EmbedBuilder()
			...
			.addFields(
				...
				{ name: '2) Horario del gremio/alianza:', value: `> ${horario}` },
				{ name: '3) ¿Por qué quieres formar alianza con Hydra?:', value: `> ${contenido}` },
				{ name: '4) Integrantes activos del gremio/alianza:', value: `> ${guild}` },
				{ name: '5) Breve resumen del contenido que se ofrece:', value: `> ${rol}` },
			);
		...
		return await Promise.all([
			...
			interaction.reply({
				content: '`✅` Tu formulario ha sido enviado correctamente a nuestro Guild Master y recibirás una respuesta pronto, sé paciente.',
				ephemeral: true,
			}),
		])
	},
